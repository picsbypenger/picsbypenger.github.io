...
def generate_column_html(image_paths):
    col_heights = [0, 0, 0]
    cols = [[], [], []]
    ...
    for path in image_paths:
        ...
        cols[idx].append((path, ratio))
        col_heights[idx] += height
        
    html = '<div class="masonry-row">\n'
    for col in cols:
        html += '    <div class="masonry-column">\n'
        for path, ratio in col:
...
print("--- SPORTS ---")
print(generate_column_html(sports_imgs))
print("\n--- PEOPLE ---")
print(generate_column_html(people_imgs))
print("\n--- LIFE ---")
print(generate_column_html(life_imgs))
