...
# Dimensions map (generated from sips output)
dims = {
    ...
    'katz1.jpg': (1365, 2048), 'katz2.jpg': (1365, 2048), # Guessed/Estimated based on aspect if missing? No, I should verify.
    'ellis2.jpg': (1365, 2048), 'ellis3.jpg': (1365, 2048),
    'dth3.jpg': (1365, 2048), 'dth4.jpg': (1365, 2048),
    'stam.jpg': (1365, 2048), 'spence.JPG': (1365, 2048), # Case sensitive
    # Life
...
# Helper to fuzzy match keys (e.g. 's/3.jpg' -> '3.jpg')
def get_dims(path):
...
for filepath in files:
    ...
    def replacer(match):
        ...
        # Extract data-src
        m = re.search(r'data-src="([^"]+)"', full_tag)
        if not m:
            return full_tag # Should not happen based on regex
        ...
        if wh:
            w, h = wh
            ratio = w / h
            # Add style, remove loading class (optional, but requested to fix bumping)
            # Actually, we keep loading class for background color? No, user wants NO bumping.
            # If we have aspect-ratio, we don't need min-height.
            return f'<figure style="aspect-ratio: {ratio}"><img class="lazy" src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" data-src="{src}" alt="."></figure>'
        else:
            return full_tag # Keep original if dimensions not found

    # Regex to find figure block
    # Matches <figure class="loading">...img...</figure>
    # Note: Handles optional whitespace
    new_content = re.sub(r'<figure[^>]*>\s*<img[^>]*data-src="([^"]+)"[^>]*>\s*</figure>', replacer, content)
